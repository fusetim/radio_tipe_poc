...
fn main() -> Result<()> {
    ...
    lora.start_receive()
        .map_err(|_| anyhow!("Failed to start Receive Continuous Mode!"))?;
    let mut stop = 0;
    while stop < 6 * 10 {
        if let Ok(true) = lora.check_receive(false) {
            let mut packet = vec![0u8; 256];
            let (rx_size, pk_info) = lora
                .get_received(&mut packet)
                .map_err(|_| anyhow!("Read packet failed!"))?;
        ...
        println!("Sending...");
        lora.start_transmit(helo_packet.as_bytes())
            .map_err(|_| anyhow!("Transmission packet failed!"))?;
        while !lora
            .check_transmit()
            .map_err(|err| anyhow!("Failed to check transmission status\ncause: {:?}", err))?
        {
            lora.delay_ms(200);
